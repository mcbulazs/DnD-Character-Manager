...
const PassivePerception: React.FC<{
	...
	const [pp, setPp] = useState<string>(value.toString());
	...
	return (
		<div
			className="h-9 w-4/5 flex flex-row items-center justify-end 
                    bg-light-parchment-beiage border-r-4 border-y-4 border-black rounded-r-3xl
                    pl-9 pr-2
                    ml-[1.5rem]
                    relative"
		>
			<div
				className="rounded-full h-10 bg-light-parchment-beiage border-4 border-black aspect-square box-content
			...
			<UnstyledNumberInput
				...
				onChange={(val) => {
					...
					if (val !== "" && !Number.isNaN(Number.parseInt(val))) {
						...
						debouncedSetPassivePerception(Number.parseInt(val));
					}
				}}
