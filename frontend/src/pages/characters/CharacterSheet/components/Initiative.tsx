...
const Initiative: React.FC<{
	...
	const [init, setInit] = useState<string>(value.toString());
				...
				<UnstyledNumberInput
					...
					onChange={(val) => {
						...
						if (val !== "" && !Number.isNaN(Number.parseInt(val))) {
							setTrueModifier(
								Math.floor((dexterity.value - 10) / 2) +
									dexterity.modifier +
									Number.parseInt(val),
							);
							debouncedSetInitiative(Number.parseInt(val));
						}
					}}
					className="text-center text-lg 
                    bg-light-parchment-beiage border-2 border-black 
